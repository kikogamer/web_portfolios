        ...
        <div id="add-block-modal" class="modal">
          ...
          <div class="modal-footer">
            ...
            <a
              ...
              id="confirm-add-block"
              @click="addBlock(blockToAdd)"
            >Adicionar</a>
          </div>
        </div>
      </div>
...
export default {
  ...
  watch: {
    blocks() {
      this.leftBlocks = this.blocks.filter(block => {
        return block.side == "left";
      });
      this.rightBlocks = this.blocks.filter(block => {
  ...
  mounted() {
    ...
    this.$resource("/portfolios{/portfolioId}/blocks")
      ...
      .then(
        ...
        response => {
          M.toast({ html: "Error on trying to get Blocks", classes: "red" });
        }
      );
  },
  ...
  methods: {
    ...
    addBlock() {
      this.$resource("/portfolios{/portfolioId}/blocks")
        .save({ portfolioId: this.portfolioId }, { block: this.blockToAdd })
        .then(
          response => {
