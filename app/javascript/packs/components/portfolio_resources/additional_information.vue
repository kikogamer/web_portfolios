  ...
  <div class="bloco-informações-adicionais">
    ...
    <div class="input-field col l12 m12 s12">
      <input
        ...
        v-model="additional_information.title"
        @change="submit()"
...
export default {
  ...
  created() {
    this.$resource(
      "/portfolios{/portfolioId}/blocks{/blockId}/additional_informations"
    )
      .get({ portfolioId: this.portfolioId, blockId: this.blockId })
      .then(
        response => {
          this.additional_information = response.body.resource;
        },
        response => {
          M.toast({
  ...
  methods: {
    submit() {
      this.$resource(
        "/portfolios{/portfolioId}/blocks{/blockId}/additional_informations{/id}"
      )
        .update(
          {
